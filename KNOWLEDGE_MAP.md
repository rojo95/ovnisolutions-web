...
- Conversión: el visitante llena el **formulario de contacto** (stepper en `components/contact-us`) que crea una *nueva solicitud de servicio* en el backend.
...
- **Servicios** (`src/app/services/`):
  - `auth/auth.service.ts` — login/logout, `isLoggedIn()` vía `localStorage.token`. **Usa `mockapi.io` hardcodeado, no `environment.backend`.**
...
```
Visitante → ContactUsComponent / FormStepper → NewServiceService.sendServiceRequest()
...
Login → AuthService.login() → GET mockapi.io (mock) → localStorage.setItem('token')
      → AuthGuard / AuthService.isLoggedIn() deciden el acceso a /dashboard
```
...
El repo está indexado (`.codegraph/`; la DB no se commitea). Antes de buscar con grep, usar desde la raíz del repo:
